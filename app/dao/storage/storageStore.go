...
func Count() (int, error) {
	...
	return activeDB.Count(&Storage_Store{})
}
...
func GetById(id any) (Storage_Store, error) {
	return GetBy(FIELD_ID, id)
}

func GetByKey(key any) (Storage_Store, error) {
	return GetBy(FIELD_Key, key)
}

func GetBy(field string, value any) (Storage_Store, error) {
	...
	if field == FIELD_ID && reflect.TypeOf(value).Name() != "int" {
		...
		return Storage_Store{}, commonErrors.WrapDAOReadError(domain, field, value, fmt.Errorf(msg, value))
	}

	if err := dao.IsValidFieldInStruct(field, Storage_Store{}); err != nil {
		return Storage_Store{}, err
	}

	if err := dao.IsValidTypeForField(field, value, Storage_Store{}); err != nil {
		return Storage_Store{}, err
	}
	...
	if err := activeDB.Retrieve(field, value, &record); err != nil {
		clock.Stop(0)
		return Storage_Store{}, commonErrors.WrapRecordNotFoundError(domain, field, fmt.Sprintf("%v", value))
	}

	if err := record.postGet(); err != nil {
		clock.Stop(0)
		return Storage_Store{}, commonErrors.WrapDAOReadError(domain, field, value, err)
	}
...
func GetAll() ([]Storage_Store, error) {

	dao.CheckDAOReadyState(domain, audit.GET, initialised) // Check the DAO has been initialised, Mandatory.

	recordList := []Storage_Store{}

	clock := timing.Start(domain, actions.GETALL.GetCode(), "ALL")

	if errG := activeDB.GetAll(&recordList); errG != nil {
		clock.Stop(0)
		return []Storage_Store{}, commonErrors.WrapNotFoundError(domain, errG)
	}
...
func GetAllWhere(field string, value any) ([]Storage_Store, error) {
	dao.CheckDAOReadyState(domain, audit.GET, initialised) // Check the DAO has been initialised, Mandatory.

	recordList := []Storage_Store{}
	resultList := []Storage_Store{}
...
func (record *Storage_Store) Spew() {
	logHandler.InfoLogger.Printf("[%v] Record=[%+v]", domain, record)
}

func (record *Storage_Store) Validate() error {
	return record.validationProcessing()
}

func (record *Storage_Store) Update(ctx context.Context, note string) error {
	return record.insertOrUpdate(ctx, note, actions.UPDATE.GetCode(), audit.UPDATE, actions.UPDATE.GetCode())
}

func (record *Storage_Store) UpdateWithAction(ctx context.Context, auditAction audit.Action, note string) error {
	return record.insertOrUpdate(ctx, note, actions.UPDATE.GetCode(), auditAction, actions.UPDATE.GetCode())
}

func (record *Storage_Store) Create(ctx context.Context, note string) error {
	return record.insertOrUpdate(ctx, note, actions.CREATE.GetCode(), audit.CREATE, actions.CREATE.GetCode())
}

func (record *Storage_Store) Clone(ctx context.Context) (Storage_Store, error) {
	return cloneProcessing(ctx, *record)
}

func (record *Storage_Store) ExportRecordAsJSON(name string) {
...
func Drop() error {
	return activeDB.Drop(Storage_Store{})
}
...
func ImportRecordsFromCSV() error {
	return importExportHelper.ImportCSV(domain, &Storage_Store{}, importProcessor)
}
