...
func (record *Report_Store) upgradeProcessing() error {
...
func (h *Report_Store) postGetProcessing() error {
...
func (record *Report_Store) preDeleteProcessing() error {
...
func reportCloneProcessing(ctx context.Context, source Report_Store) (Report_Store, error) {
	//TODO: Add any clone processing here
	panic("Not Implemented")
	return Report_Store{}, nil
}

func reportJobProcessor(j jobs.Job) {
	...
	ReportEntries, err := GetAll()
	if err != nil {
		logHandler.ErrorLogger.Printf("[%v] Error=[%v]", jobs.CodedName(j), err.Error())
		return
	}

	noReportEntries := len(ReportEntries)
	if noReportEntries == 0 {
		logHandler.ServiceLogger.Printf("[%v] No %vs to process", jobs.CodedName(j), domain)
		clock.Stop(0)
		return
	}

	for ReportEntryIndex, ReportRecord := range ReportEntries {
		logHandler.ServiceLogger.Printf("[%v] %v(%v/%v) %v", jobs.CodedName(j), domain, ReportEntryIndex+1, noReportEntries, ReportRecord.Raw)
