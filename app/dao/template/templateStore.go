...
func Count() (int, error) {
	...
	return activeDB.Count(&Template_Store{})
}
...
func GetById(id any) (Template_Store, error) {
	return GetBy(FIELD_ID, id)
}

func GetByKey(key any) (Template_Store, error) {
	return GetBy(FIELD_Key, key)
}

func GetBy(field string, value any) (Template_Store, error) {
	...
	if field == FIELD_ID && reflect.TypeOf(value).Name() != "int" {
		...
		return Template_Store{}, commonErrors.WrapDAOReadError(domain, field, value, fmt.Errorf(msg, value))
	}

	if err := dao.IsValidFieldInStruct(field, Template_Store{}); err != nil {
		return Template_Store{}, err
	}

	if err := dao.IsValidTypeForField(field, value, Template_Store{}); err != nil {
		return Template_Store{}, err
	}

	record := Template_Store{}
	logHandler.DatabaseLogger.Printf("Get %v where (%v=%v)", domain, field, value)

	if err := activeDB.Retrieve(field, value, &record); err != nil {
		clock.Stop(0)
		return Template_Store{}, commonErrors.WrapRecordNotFoundError(domain, field, fmt.Sprintf("%v", value))
	}

	if err := record.postGet(); err != nil {
		clock.Stop(0)
		return Template_Store{}, commonErrors.WrapDAOReadError(domain, field, value, err)
	}
...
func GetAll() ([]Template_Store, error) {

	dao.CheckDAOReadyState(domain, audit.GET, initialised) // Check the DAO has been initialised, Mandatory.

	recordList := []Template_Store{}

	clock := timing.Start(domain, actions.GETALL.GetCode(), "ALL")

	if errG := activeDB.GetAll(&recordList); errG != nil {
		clock.Stop(0)
		return []Template_Store{}, commonErrors.WrapNotFoundError(domain, errG)
	}
...
func GetAllWhere(field string, value any) ([]Template_Store, error) {
	dao.CheckDAOReadyState(domain, audit.GET, initialised) // Check the DAO has been initialised, Mandatory.

	recordList := []Template_Store{}
	resultList := []Template_Store{}
...
func (record *Template_Store) Spew() {
	logHandler.InfoLogger.Printf("[%v] Record=[%+v]", domain, record)
}

func (record *Template_Store) Validate() error {
	return record.validationProcessing()
}

func (record *Template_Store) Update(ctx context.Context, note string) error {
	return record.insertOrUpdate(ctx, note, actions.UPDATE.GetCode(), audit.UPDATE, actions.UPDATE.GetCode())
}

func (record *Template_Store) UpdateWithAction(ctx context.Context, auditAction audit.Action, note string) error {
	return record.insertOrUpdate(ctx, note, actions.UPDATE.GetCode(), auditAction, actions.UPDATE.GetCode())
}

func (record *Template_Store) Create(ctx context.Context, note string) error {
	return record.insertOrUpdate(ctx, note, actions.CREATE.GetCode(), audit.CREATE, actions.CREATE.GetCode())
}

func (record *Template_Store) Clone(ctx context.Context) (Template_Store, error) {
	return cloneProcessing(ctx, *record)
}

func (record *Template_Store) ExportRecordAsJSON(name string) {
...
func Drop() error {
	return activeDB.Drop(Template_Store{})
}
...
func ImportRecordsFromCSV() error {
	return importExportHelper.ImportCSV(domain, &Template_Store{}, importProcessor)
}
